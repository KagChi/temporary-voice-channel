...
let clientListener = class clientListener extends framework_1.Listener {
    run({ context, message: content }, { message }) {
        if (Reflect.get(Object(context), "silent"))
            return;
        return message.channel.send({ embeds: [new discord_js_1.MessageEmbed().setDescription(content)], allowedMentions: { users: [message.author.id], roles: [] } });
    }
};
clientListener = __decorate([
    (0, decorators_1.ApplyOptions)({
        name: framework_1.Events.CommandDenied
    })
], clientListener);
