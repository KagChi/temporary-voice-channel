...
let readyEvent = class readyEvent extends framework_1.Listener {
    async run(oldState, newState) {
        ...
        const parentVoiceChannel = this.container.client.channels.resolve(config_1.parentTempVoiceId);
        if (parentVoiceChannel && parentVoiceChannel.isVoice()) {
            ...
            if (newState.channelId && newState.channelId === config_1.parentTempVoiceId) {
                ...
                if (TemporaryChannelDatabase) {
                    if (!this.container.client.channels.cache.has(TemporaryChannelDatabase.channelId)) {
                        await newState.member?.voice.setChannel(config_1.parentTempVoiceId);
                ...
                this.container.client.tempVoiceManager.setUserChannel(newState.member?.id, {
                    ...
                    parentChannelId: config_1.parentTempVoiceId,
